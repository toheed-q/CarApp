-- ============================================================================
-- Migration: allow the car filter to match MULTIPLE brands at once.
--
-- The app now sends @ByBrand as a comma-separated list (e.g. 'Tata,Toyota').
-- Changes vs the previous proc:
--   1. @ByBrand widened VARCHAR(100) -> VARCHAR(500) to hold several names.
--   2. Brand match uses IN (STRING_SPLIT(...)) instead of a single equality.
-- A single brand (no comma) still works exactly as before. NULL = no filter.
--
-- Safe to run repeatedly (ALTER). Run on the ACD database.
-- ============================================================================
ALTER PROCEDURE [dbo].[GetCars]
(
    @ByBrand          VARCHAR(500)  = NULL,
...
BEGIN
    ...
    WHERE 1 = 1
        AND (@ByCarId         IS NULL OR cd.ID           = @ByCarId)
        -- Multi-brand: @ByBrand is a comma-separated list; NULL = no brand filter.
        AND (@ByBrand         IS NULL OR cd.Brand IN
                (SELECT LTRIM(RTRIM(value)) FROM STRING_SPLIT(@ByBrand, ',') WHERE LTRIM(RTRIM(value)) <> ''))
